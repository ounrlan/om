/**
 * Google Yorumları — Places API (New) üzerinden canlı veri çekme.
 *
 * Kurulum (bkz. README.md):
 *  1. Google Cloud Console'da bir proje açın, "Places API (New)" etkinleştirin.
 *  2. Bir API anahtarı oluşturun ve `.env.local` dosyasına ekleyin:
 *       GOOGLE_MAPS_API_KEY=AIza...
 *       GOOGLE_PLACE_ID=ChIJ...   (opsiyonel — yoksa işletme adıyla otomatik bulunur)
 *  3. Sunucuyu yeniden başlatın. Yorumlar 6 saatte bir tazelenir.
 *
 * Anahtar tanımlı değilse `null` döner ve sitede src/data/site.ts içindeki
 * TESTIMONIALS yedek listesi gösterilir.
 */

const REVALIDATE_SECONDS = 21600; // 6 saat
const SEARCH_QUERY = "UYG GOLD ESTATE Bağlıca Etimesgut Ankara";
