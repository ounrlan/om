...
export default async function GoogleReviews() {
  ...
  return (
    <section id="yorumlar" className="relative overflow-hidden bg-olive-deep/30 py-24 sm:py-32">
      ...
      <div className="container-luxe">
        <div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
          ...
          <Reveal delay={0.15} className="mb-14 shrink-0">
            <div className="glass flex items-center gap-5 rounded-card px-7 py-5">
              <GoogleG size={34} />
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-display text-3xl font-semibold text-cream">
                    {(live?.rating ?? GOOGLE.rating).toFixed(1).replace(".", ",")}
                  </span>
                  <Stars rating={live?.rating ?? GOOGLE.rating} />
                </div>
                <p className="mt-1 text-xs text-stone">
                  Google&apos;da {live?.count ?? GOOGLE.count} değerlendirme
                </p>
              </div>
            </div>
          </Reveal>
            ...
            <Reveal key={r.author + i} delay={i * 0.1}>
              <figure className="card-luxe flex h-full flex-col p-7">
                ...
                <figcaption className="mt-6 flex items-center gap-3 border-t border-white/[0.07] pt-5">
                  <span className="flex h-10 w-10 items-center justify-center rounded-full bg-gold/15 font-display text-sm font-semibold text-gold">
