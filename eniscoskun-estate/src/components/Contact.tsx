...
const ITEMS: ContactItem[] = [
  ...
  },
  {
    ...
    rows: [
      { day: "Hafta içi & Cumartesi", time: "09:00 – 19:00" },
    ...
    <section id="iletisim" className="relative py-24 sm:py-32">
      <div className="container-luxe">
        <SectionHeading
          ...
          description="Portföyümüz, mülkünüzün değerlemesi ya da yatırım planlarınız için Eryaman'daki ofisimize bekleriz."
        />
        ...
        <div className="grid gap-10 lg:grid-cols-2">
          <div className="grid content-start gap-4 sm:grid-cols-2">
            {ITEMS.map((item, i) => {
              const inner = (
                <>
                  <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-gold/25 bg-gold-pale/60 text-gold-deep">
                    <item.icon size={20} strokeWidth={1.6} />
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block text-[11px] font-semibold uppercase tracking-luxe text-ash">
                      {item.label}
                    </span>
                    {item.rows ? (
                      <span className="mt-2 block space-y-1.5">
                        {item.rows.map((r) => (
                          ...
                          >
                            <span className="text-[13px] text-ash">{r.day}</span>
                            <span className="mx-1 hidden h-px flex-1 self-center bg-mist sm:block" />
              ...
              const cls = "card flex items-center gap-4 p-5 sm:p-6 h-full";
              return (
            ...
            <Reveal delay={0.42} className="sm:col-span-2">
              <a
                href={SAHIBINDEN.store}
                target="_blank"
                rel="noopener"
                className="card flex items-center justify-between gap-4 p-5 sm:p-6"
              >
          ...
          <Reveal delay={0.2}>
            <div className="relative h-full min-h-[420px] overflow-hidden rounded-card border border-mist shadow-card">
              <iframe
                src={CONTACT.mapsEmbed}
                title="Enis Coşkun Gayrimenkul ofis konumu"
                loading="lazy"
